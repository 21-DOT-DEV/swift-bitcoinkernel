...
- If `tuist test` reports "the scheme's test action has no tests to run" (or `xcodebuild test-without-building` says the scheme "is not currently configured"): re-run `swift package --disable-sandbox tuist generate -p Projects/ --no-open` from the repo root — observed to restore the scheme's `TestableReference`s after `tuist test` had produced an empty test action — then use the CI `xcodebuild` path below.
...
Both `NodeApp` and `KernelApp` declare `sources: ["Sources/<App>/**", "Sources/Shared/**"]` in `Project.swift`, so the shared Tor view-model (`TorViewModel.swift`), the SwiftUI status view (`TorStatusView.swift`), the `URLSession`-over-Tor helper (`URLSessionConfiguration+Tor.swift`), and the node-diagnostics utility (`NodeDiagnostics.swift`) end up duplicated in each app's binary at compile time. Tests follow the same pattern with `Sources/SharedTests/**` (`FakeTorSession`, `SessionFactory`, `WaitFor`).

**SharedTests discriminator: per-bundle `-D` flags, never `canImport`.** Files under `Sources/SharedTests/` compile into both test bundles and pick the app module with `#if NODEAPP_TESTS / #elseif KERNELAPP_TESTS` around the `@testable import`. `Project.swift` defines those flags in each test target's `base` settings (next to the apps' `Xcode` flag). `canImport` cannot discriminate here: every target's `-I` path includes the workspace's shared products dir, so once `NodeApp` has been built into a derived data folder, `NodeApp.swiftmodule` is visible to `KernelAppTests` too — `canImport(NodeApp)` would evaluate true there, the file would import the wrong module, and the scan would fail with `Unable to resolve module dependency: 'bitcoind'` (that target has no `bitcoind` modulemap). New `SharedTests` files must use the flags; a file landing in a bundle with neither defined fails fast via `#error`.

`KernelAppTests` additionally reaches *up* to two files in the package's own test helpers (`../Tests/BitcoinKernelTests/Support/{KernelSerialization,AsyncSemaphore}.swift`), listed as explicit paths rather than a wildcard because that folder also holds a `MockBlockSource.swift` that would collide with this bundle's own. `WaitFor` polls the REAL clock and currently has zero callers — prefer injecting a `TestClock` into `TorViewModel(clock:)` or `KernelAppViewModel(clock:)`; see the note at the top of `WaitFor.swift`. When adding a file to `Sources/Shared/`, both apps pick it up automatically — no `Project.swift` edit needed.
