...
/// without a node, a screen, or the address-hiding network. `NodeRun` orchestrates
/// and does no deciding; every judgment it appears to make is one of these. See
...
enum NodeAutomation {
    ...
    static func summary(
        ...
        case .alreadyRunning, .started:
            ...
            var sentence = "\(verb) on \(chain) at block \(height)"
            if let blocksBehind, blocksBehind > 0 { sentence += ", \(blocksBehind) behind" }
            sentence += "."
            if let gained = blocksSinceLastCheck {
                sentence +=
                    gained == 0
                    ? " No new blocks since the last check."
                    : " \(gained) new block\(gained == 1 ? "" : "s") since the last check."
            }
            return sentence
        }
    }
