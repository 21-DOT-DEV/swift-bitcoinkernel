...
//
// `shutdownReleasesDataDirectoryBeforeReopen` — the regression test for the
// stop-then-reopen race that `cancellationEndsSequenceSilently` above did not
// catch — lives in `BlockchainSyncTeardownTests.swift`. It needs neither
// CryptoKit nor `RegtestChainBuilder`, so it sits outside this file's gate and
// runs on Linux too.
