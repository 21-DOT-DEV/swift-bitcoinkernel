...
The daemon runs until shutdown is signaled (via ``Daemon/stop()`` or the `stop` RPC), at which point `bitcoind_main` returns and the completion semaphore is signaled.
