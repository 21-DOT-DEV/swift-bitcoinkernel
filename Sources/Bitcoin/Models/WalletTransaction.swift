...
public struct WalletTransaction: Codable, Sendable, Equatable {
    /// The amount of the transaction in BTC.
    public let amount: BTCAmount

    /// The fee in BTC (negative, only for send transactions).
    ...
    /// The time the transaction was replaced (BIP 125).
    public let replaced_by_txid: String? // swiftlint:disable:this identifier_name

    /// The time the transaction replaces another.
    ...
    public let details: [WalletTransactionDetail]?

    /// The serialized, hex-encoded transaction data.
    public let hex: String?

    /// The decoded transaction (when verbose=true).
    ...
    enum CodingKeys: String, CodingKey {
        case amount, fee, confirmations, generated, trusted
        case blockhash, blockheight, blockindex, blocktime
        case txid, wtxid, walletconflicts
        case replaced_by_txid, replaces_txid // swiftlint:disable:this identifier_name
        case mempoolconflicts, to, time, timereceived, comment
        case parent_descs // swiftlint:disable:this identifier_name
        case details, hex, decoded, lastprocessedblock
        case bip125Replaceable = "bip125-replaceable"
    }
}

/// Per-category detail of a wallet transaction.
public struct WalletTransactionDetail: Codable, Sendable, Equatable {
    ...
    /// List of parent descriptors for the output script (only for "received" category).
    public let parent_descs: [String]? // swiftlint:disable:this identifier_name
}
