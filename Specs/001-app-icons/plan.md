# Plan — App icons (Icon Composer / Liquid Glass)

| | |
|---|---|
| **Feature** | New app icons for the two example apps — `NodeApp` and `KernelApp` — built from three supplied SVGs, using Apple's `.icon` (Icon Composer / Liquid Glass) format so they show the layered look on iOS/macOS 26 and a compiler-flattened version down to the apps' floor (iOS 18 / macOS 15). Demo-app launch polish, **not a library roadmap phase** (the roadmap tracks the `Bitcoin`/`BitcoinKernel` library; see [`.specify/memory/roadmap.md`](../../.specify/memory/roadmap.md)). This repo's first Spec — global counter starts at 001. |
| **Status** | **IN PROGRESS — Phases 0–4 done, awaiting your Phase 5 polish.** Pre-implementation questions all resolved (authoring path C — agent hand-authors, you validate+polish; targeted validation A; full drawing + one design across platforms; delete placeholders; accept auto-flattened fallback). Agent work complete on branch `APP-ICONS`, **uncommitted**: both `AppIcon.icon` bundles authored (§3.2), wired into `Project.swift`, and the two empty `AppIcon.appiconset` placeholders removed. Validated: `tuist generate` succeeds and the generated project references the bundles; `actool` compiles both clean for iOS 18 and macOS 15 (`Assets.car` produced + older-OS fallback keys `CFBundleIconName`/`AppIcon60x60` generated, confirming D5). Next: open each `.icon` in Icon Composer to validate + polish (Phase 5), then commit. |
| **Decisions** (planning) | **D1 — Format (`.icon`).** One Icon Composer `.icon` bundle per app; the build compiler (`actool`) generates every size and the older-OS fallback from it, so no SVG rasterizer is installed. Chosen because the local machine and CI already run Xcode 26 (`macos-26` / 26.4), it ingests the SVGs directly, and it is Apple's current format. **D2 — One node icon now.** Ship a single fixed NodeApp icon; runtime mode-switching (archival vs pruned) is a clean, no-rework follow-up (§8). **D3 — Shared warm background.** A cream→sand linear gradient behind all icons, art flat on top, so the two apps read as a family and the rust node line-art keeps dark-on-light contrast; researched against Apple HIG + Liquid Glass color guidance (avoid pure white, prefer a subtle gradient, don't bake gloss/shadow into the art). **D4 — Floor unchanged.** Deployment target stays iOS 18 / macOS 15; only the Xcode-26 build toolchain is required, already in place. **D5 — Older-OS fallback is take-it-or-leave-it.** Older OSes get Xcode's auto-flattened version of the same `.icon`; Apple does not support a separate developer-controlled legacy icon alongside a `.icon` (confirmed "by design"). Accept it and verify at the floor; if it were ever unacceptable, the only recourse is to defer `.icon` and keep asset catalogs — not a hand-tuned fallback image. |
| **Assumptions** (in force) | **A1 —** Spec lives at `Specs/001-app-icons/plan.md` only; `Specs/README.md` seeded as the index (this repo's first spec; phase mapping in the header, not the path). **A2 —** The full/archival drawing (`node_app_full.svg`) is the NodeApp icon; `node_app_pruned.svg` is kept for the switching follow-up. **Open to veto.** **A3 —** One design covers iPhone/iPad/Mac; no separate Mac artwork (verified in the build step). **Open to veto.** **A4 —** `ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon` is already set in `Projects/Resources/{KernelApp,NodeApp}/Shared.xcconfig`, so the bundle is named `AppIcon.icon` and no build-setting edit is added. **A5 —** Verified with `tuist generate` + `xcodebuild` on both platforms (matches CI); the SVGs contain no live text (`<text>`/`<tspan>` count 0), so no outline-conversion step. |
...
## 7. Review resolutions (2026-07-16)

Resolves the code review of this plan (flight recorder `review-findings-69B3599C`).

- **[Finding 1 — BUG, `icon.json` schema] FIXED.** §3.2 uses the real schema: top-level `fill` with `linear-gradient` (array of `srgb:`/`display-p3:` decimal-RGBA strings) + `orientation`, and `groups[].layers[]` for the art. Hex converted to `srgb:` decimals. Evidence: real `icon.json` from `AllanChain/sane-break` commit `a0cd678`; `skiptools/skip` issue #443.
- **[Finding 2 — BUG, fallback contingency] FIXED.** Removed the "add a populated 1024px fallback" step. Decision D5 + Phase 4 now state older OSes get Xcode's auto-flattened `.icon`; a separate legacy icon is unsupported. Evidence: Apple "Creating your app icon using Icon Composer" ("replaces any existing icon asset catalog"; "generates a similar-looking version… for previous releases"); Michael Tsai 2025-08-08 (Apple: "by design").
- **[Finding 3 — LOG, SVG live text] FALSE POSITIVE.** Verified 0 `<text>`/`<tspan>` in all three SVGs; Phase 1 records the check (A5).
- **[Candidate 5 — duplicate build setting] FIXED.** `ASSETCATALOG_COMPILER_APPICON_NAME` is already set in both `Shared.xcconfig` files (A4); the plan no longer adds it in `Project.swift`.
- **[Candidate 6 — resource path] FIXED.** Phase 3 specifies the `.icon` as a directory-path resource, not a `/**` glob.
- **[Candidate 2 — Icon Composer runtime] NOTED.** Requires macOS 26.4+; local is 26.5.1.
