...
- **Xcode sessions**: when running inside Xcode's coding assistant, always drive Xcode through the `xcode-tools` MCP tools (`BuildProject` for builds, `RunAllTests`/`RunSomeTests` for tests, `XcodeGrep`/`XcodeGlob`/`XcodeRead`/`XcodeWrite` for files, `DocumentationSearch` for docs) instead of shell commands or built-in tools — they are pre-approved (no permission prompts) and Xcode executes them natively, rendering rich result cards. This also keeps heavy output off the ACP terminal path: a full `xcodebuild` log through the chat terminal has crashed Xcode 27.0. If a shell build is truly necessary, redirect to a file and `tail` it instead of streaming.
